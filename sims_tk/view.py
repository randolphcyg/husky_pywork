...
pic = os.path.join(ROOT_PATH, 'head.jpg')
...
class InputFrame(Frame):                # 继承 Frame 类
    ...
    def save(self):
        ...
        if num in nums_list:
            ...
            tkinter.messagebox.showinfo(title="信息", message="信息登记更新并保存成功！")
        else:
            df.loc[row] = [num, name, cla, gender, sub1, sub2, sub3, sub4, sub5, sub6]
            pd.DataFrame(df).to_excel(output_file, sheet_name="学生信息", header=True, index=False)
            tkinter.messagebox.showinfo(title="信息", message="信息登记保存成功！")
...
class DelFrame(Frame):
    ...
    def update_csv_file(self, df):
        ...
        if confirm:
            ...
            tkinter.messagebox.showinfo(title="信息", message="信息删除成功！")
